...
public class SchoolController:MonoBehaviour{
	...
	public int _childAmount = 0;				// Number of objects
    //add by YY 2016-3-24
    public float _liveTotalTime = 0f;            //The Fish Live Time in CurrentScene 
    public Color _colSchoolCtrl = Color.red;  //Gizemo Color;
    private float _curLiveTime = 0f;
    //end by YY 2016-3-24
	...
	public float _minScale = 0.7f;				// minimum random size
	...
	public float _acceleration = 0.025f;			// How fast child speeds up
	public float _brake = 0.01f;					// How fast child slows down 
	...
	public void Update() {
		if(_activeChildren > 0){
			...
			UpdateFishAmount();
            _curLiveTime += Time.deltaTime;
            if (_curLiveTime > _liveTotalTime&&_liveTotalTime>0)
            {
                MannulUpdateWaypoint();
            }
            else
            {
	...
	//add by YY 2016-3-24
    public void MannulUpdateWaypoint()
    {
        CancelInvoke("AutoRandomWaypointPosition");
        
        _posBuffer = new Vector3(0,1.5f,5f);// Vector3.forward * Time.deltaTime + Vector3.up * Time.deltaTime;
    
        if (_forceChildWaypoints)
        {
            for (int i = 0; i < _roamers.Count; i++)
            {
                (_roamers[i]).SetWaypointState(false);
            }
        }
	...
	public void OnDrawGizmos() {
        ...
        Gizmos.color = _colSchoolCtrl;// Color.cyan;
