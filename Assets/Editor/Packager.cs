...
public class Packager {
    ...
    static UnityEngine.Object LoadAsset(string file) {
        ...
        return AssetDatabase.LoadMainAssetAtPath("Assets/Prefabs/" + file);
    }
    ...
    static void HandleExampleBundle(BuildTarget target) {
        ...
        ///------------------------------生成LoginPanel素材绑定-----------------------------------
        //BuildPipeline.PushAssetDependencies();
        //mainAsset = LoadAsset("Login/LoginPanel.prefab");
        ////addis = new Object[1];
        ...
        /////------------------------------生成MessagePanel素材绑定-----------------------------------
        //BuildPipeline.PushAssetDependencies();
        //mainAsset = LoadAsset("Hall/HallPanel.prefab");
        //assetfile = assetPath + "hall" + AppConst.ExtName;
        //BuildPipeline.BuildAssetBundle(mainAsset, null, assetfile, options, target);
        //BuildPipeline.PopAssetDependencies();
        string prefabPaths= AppDataPath+"/Prefabs/";     
 
         files.Clear();
         Recursive(prefabPaths);
         int n = 0;
         foreach (string f in files)
         {
             if (!f.EndsWith(".prefab")) continue;
             string newfile = f.Replace(prefabPaths, "");
             string fileName = Path.GetFileName(f);
             string fileNoExtenion = fileName.Split('.')[0];

             BuildPipeline.PushAssetDependencies();
             mainAsset = LoadAsset(newfile);
             assetfile = assetPath + fileNoExtenion + AppConst.ExtName;
             BuildPipeline.BuildAssetBundle(mainAsset, null, assetfile, options, target);
             BuildPipeline.PopAssetDependencies();
