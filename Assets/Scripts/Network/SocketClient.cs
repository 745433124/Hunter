...
public class SocketClient {
    ...
    private const Int32 MAX_READ = 1024 * 1024;
    ...
    void OnConnect(IAsyncResult asr) {
        ...
        NetworkManager.AddEvent("Connected");
    }
    ...
    void OnReceivedMessage(MemoryStream ms) {
        ...
        string pkg = m_decode(message);
        //Debug.Log("pkg:" + pkg);
        NetworkManager.AddEvent(pkg);
    }
    ...
    /// <returns></returns>
    public static string m_decode(byte[] value)
    {
        string model = Encoding.UTF8.GetString(value);
        return model;
    }
    ...
    private int Send(byte[] packBuff)
    {
        try
        {
            uint len = (uint)packBuff.Length;
            uint offset = 0;

            while (len != 0)
            {
                int send = client.Client.Send(packBuff, (int)offset, (int)len, SocketFlags.None);
                if (send <= 0)
                {
                }
                offset += (uint)send;
